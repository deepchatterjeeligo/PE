...
class FrequencyDomainWaveformLoader(WaveformSampler):
    """
    Torch module for loading waveforms from disk,
    performing train/val/test split, and sampling
    them during training.
    ...
    @property
    def val_waveforms_per_device(self, world_size=1, rank=0):
        """Number of validation waveforms per device"""
        return self.num_val_waveforms // world_size
    ...
    def load_signals(self, start, stop):
        ...
        with h5py.File(self.waveform_file) as f:
            signals = {
                "cross": torch.from_numpy(f["waveforms"]["cross"][start:stop]),
                "plus": torch.from_numpy(f["waveforms"]["plus"][start:stop]),
            }
            parameters = {}
            for parameter in self._inference_params:
                parameters[parameter] = torch.from_numpy(
                    f["parameters"][parameter][start:stop]
                )
    ...
    def get_val_waveforms(self, rank=0, world_size=1):
        """
        Returns validation waveforms for this device
        """
        start, stop = self.get_slice_bounds(
    ...
    def sample(self, N):
        ...
        Sample method for generating training waveforms
        """
        idx = torch.randperm(len(self.train_waveforms["plus"]))[:N]

        cross = self.train_waveforms["cross"][idx]
        plus = self.train_waveforms["plus"][idx]
        parameters = {}
        for k, v in self.train_parameters.items():
            parameters[k] = v[idx]
        polarizations = {"cross": cross, "plus": plus}
